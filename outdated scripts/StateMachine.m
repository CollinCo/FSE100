    ...
    switch state
        case INITIALIZING
            ...
            clutch = Clutch(brick, 2, -1); % init clutch obj
                %cl?tch.set(-1);    % the clutch is set in reverse mode
                                    % wheels will turn in opposite directions.
                %cl?tch.set(1);     % the clutch is set in forward mode
        ...
        case CHECKING
            ...
            if wall_right and wall_front:
                if(wall_right && wall_front)
                state = TURNING_LEFT;
            else if wall_right and ~wall_front:
        ...
        case MOVING_FORWARD
            ...
            if move_interrupted and floor_color == green and picked_up == false:
                state = BACKING_UP;
            else if move_interrupted and floor_color == green and picked_up == true:
                state = DROPPING_OFF;
            else if move_interrupted:
                state = PICKING_UP;
            else:
                state = CHECKING;                                
