classdef bricksimApp < handle
    ...
    properties (Constant)
        AppPathBase = {'/bricksim/code'};
        AppClass = 'bricksimApp';
        Increment = 1;
        Decrement = 0;
        Version = '13a';
    end
    ...
    methods
        ...
        function startApp(obj)
            ...
            mlock;
            bricksimApp.refcount(obj.Increment);
            ...
            existVal = exist(fullfile(pwd,'simBrickServer')); %#ok<EXIST>
            ...
            if (doesShadowExist && ~pathOne.equals(pathTwo))
                % We are trying to execute the wrong simBrickServer
                errordlg(message('MATLAB:apps:runapp:WrongEntryPoint', 'simBrickServer').getString, ...
            ...
            if nargout(@simBrickServer) == 0
                eval('simBrickServer');
            else
                obj.AppHandle = eval('simBrickServer');
            end
