...
% DS ADDED outputPolarity  Sets the polarity of a specified output
...
% DS ADDED outputStepSync moves two or more motors in sync with each other
...
classdef Brick < handle
    ...
    methods
        ...
        function reading = ColorReflect(brick, SensorPort)
            reading = brick.inputReadSI(SensorPort, Device.ColReflect);
        end
        
        function reading = ColorAmbient(brick, SensorPort)
        ...
        function motorPolarity(brick,nos,polarity)
            % ADDED BY DS
            % Brick.motorPolarity Set the motor polarity
            % 
            % Notes::
            %
            % - NOS is a bit field representing output 1 to 4 (0x01, 0x02, 0x04, 0x08).
            % - polarity is the motor polarity (-1=backward, 1= forward, 0=toggle).
            %
            % Example::
            %           b.outputPower(0,Device.MotorA,50)
            nos = makenos(nos);
            cmd = Command();
            cmd.addHeaderDirect(42,0,0);
            cmd.opOUTPUT_POLARITY(0,nos,polarity);
            cmd.addLength();
            brick.send(cmd);
        end
        ...
        function motorStepSync(brick,nos,speed,step,ratio,brake)
            % ADDED BY DS
            % Brick.outputStepSync Output a step speed 
            %
            % Brick.outputStepSpeed(nos,speed,step1,step2,step3,brake)
            % moves a motor to set position with layer, NOS, speed, ramp up
            % angle, constant angle, ramp down angle and brake.
            ...
            % - NOS is a bit field representing output 1 to 4 (0x01, 0x02, 0x04, 0x08).
            % - speed is the output speed with [+-0..100%] range.
            % - ratio is the turn ratio for multiple motors.
            % - step  is the steps used for constant speed.
            % - brake is [0..1] (0=Coast,  1=Brake).
            %
            nos = makenos(nos);
            brake = makebrake(brake);
            cmd = Command();
            cmd.addHeaderDirect(42,0,0);
            cmd.opOUTPUT_STEP_SYNC(0,nos,speed,ratio,step,brake);
            cmd.addLength();
            brick.send(cmd);
        end
